...
//===========================Incident===========================
  ...
  const updateIncidentDescription = async (incidentId, updatedDescription) => {
    try {
      ...
      const response = await fetch(`https://smart-city-backend-301261782088.asia-south1.run.app/${incidentId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
  ...
  const createIncident = async () => {
    try {
      ...
      const response = await fetch('https://smart-city-backend-301261782088.asia-south1.run.app/', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
  ...
  const fetchIncidentData = async () => {    
    try {
      const token = localStorage.getItem("token");
      const res = await fetch("https://smart-city-backend-301261782088.asia-south1.run.app/", {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${token}`,
        }
