...
//===========================Accident History===========================
  ...
  const fetchAccidentHistory = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('https://smartcity-backend-try1-301261782088.asia-south1.run.app/accident-history/all', {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
...
const updateAccidentHistory = async () => {
  try {
    ...
    const response = await fetch(`https://smartcity-backend-try1-301261782088.asia-south1.run.app/accident-history/${oldVID}/${oldIID}?old_vid=${oldVID}&old_iid=${oldIID}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`,
