...
const bookVehicle = async (vehicleId) => {
  try {
    const token = localStorage.getItem("token");
    const res = await fetch(`https://smart-city-backend-301261782088.asia-south1.run.app/${vehicleId}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`
...
const toggleVehiclesModal = async () => {
  ...
  try {
    const token = localStorage.getItem("token");
    const res = await fetch("https://smart-city-backend-301261782088.asia-south1.run.app/all", {
